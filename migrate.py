...
concept_codes = { "TRADITIONAL + PAN MQ + COMPOSITE + STEPPING": "TRF", "OPEN CLOSE": "OPC", "COMPOSITE": "COM", "STRUCTURE": "STR", "PAN MQ (SINGLE DOUBLE ROUND)": "PANMQ", "TRADITIONAL REGULAR": "TR", "LAYERS": "LAY", "MODERN / CONTEMPORARY": "MOC", "COLOURSTONE": "COL", "TAAR SPREADLOOK": "TSL", "MISCELLANEOUS": "MISC", "FULL SET": "FS" }
...
def update_google_sheets(t2_rows):
    print("Syncing Database Status to Google Sheets...")
    try:
        comp_sheet = spreadsheet.worksheet("COMPILATION")
        current_values = comp_sheet.get_all_values()
        
        # Store your manual 'ON HOLD' or 'CANCELLED' notes
        manual_overrides = {}
        ...
        for r in t2_rows:
            ...
            # PROTECT YOUR MANUAL NOTES (Column I & J)
            ...
            # STATUS (Column H) now trusts the SQL database result
            # But if you manually put it ON HOLD, we show that instead
            display_status = r['status']
            if final_admin == 'ON HOLD':
                display_status = 'ON HOLD'
            elif final_admin == 'CANCELLED':
                display_status = 'CANCELLED'

            comp_data.append([
                r['designer_code'], d_no, r['budget'], r['order_date'], 
                r['order_type'], r['priority'], r['remark'], 
                display_status, # Trust BigQuery status
                final_admin, final_archive
            ])
            
        if comp_data:
            comp_sheet.update(range_name="A2", values=comp_data)
            print("✅ COMPILATION and Designer Tabs updated.")
